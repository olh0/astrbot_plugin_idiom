...
# 成语列表
IDIOM_LIST = [
    "好高骛远", "有恃无恐", "淋漓尽致", "愤世嫉俗", "涣然冰释", "庸人自扰", "引吭高歌", "老奸巨猾", "略见一斑", "味同嚼蜡",
    "绵里藏针", "一语破的", "买椟还珠", "匠心独运", "安之极素", "刚愎自用", "弱不禁风", "门可罗雀", "动辄得咎", "莫衷一是",
    "为虎作伥", "痴心妄想", "不三不四", "事半功倍", "委曲求全", "蔚然成风极", "汗牛充栋", "文过饰非", "明日黄花", "如法炮制",
    "满面春风", "侃侃而谈", "包罗万象", "鬼鬼祟祟", "阴谋诡计", "迫在眉睫", "鳞次栉比", "绝无仅有", "细大不捐", "融会贯通",
    "众口铄金", "返璞归真", "韬光养晦", "生灵涂炭", "极俊不禁", "美轮美奂", "志同道合", "纵横捭阖", "耳闻目睹", "蔚为大观",
    ...
    "目不暇接", "秘而不宣", "浮想联翩", "按兵不动", "再接再厉", "不刊之论", "迫不及待", "无可非议", "风声鹤唳", "厚此薄彼",
    ...
    "开门揖盗", "走投无路", "急如星火", "各抒己见", "薪尽火传", "文不加点", "路有饿殍", "毛骨悚极", "颐指气使", "堂而皇之",
    "流连忘返", "既往不咎", "一言九鼎", "余音绕梁", "独树一帜", "指手画脚", "临渊羡鱼", "亘古未有", "微不足道", "升堂入室",
    "箪食壶浆", "肆无忌惮", "白璧微瑕", "鸿篇巨制", "有口皆碑", "励精图治", "炙手可热", "纵横驰骋", "唉声叹气极", "博闻强识",
    "慷慨激昂", "差强人意", "怙恶不悛", "一针见血", "百无聊赖", "刻不容缓", "粗茶淡饭", "想入非非", "临深履薄", "病入膏肓",
    "有口皆碑", "责无旁贷", "声名狼藉", "味同嚼蜡", "曲高和寡", "诚惶诚恐", "休戚相关", "望其项背", "娓娓动听", "以逸待劳",
    "迥然不同", "左右逢源", "一往无前", "所向披靡", "剑拔弩张", "浮想联翩", "应接不暇", "凤毛麟角", "一意孤行",极"语无伦次",
    ...
    "失之交臂", "焕然一新", "相濡以沫", "司空见惯", "义不容辞", "斑驳陆离", "功败垂成", "洗心革面", "迫不及待", "付之一笑",
    "厉行节约", "相敬如宾", "间不容发", "不负众望", "艰苦奋斗", "匪夷所思", "不孚众望", "相形见绌", "蓬荜生辉", "附庸风雅",
    "信手拈来", "安步当车", "畸形发展", "火中取栗", "适逢其会", "蛊惑人心", "另眼极看", "如雷贯耳", "虚怀若谷", "绘声绘色",
    "触类旁通", "矫揉造作", "一丘之貉", "得不偿失", "偃旗息鼓", "目不斜视", "功亏一篑", "灯红酒绿", "捉襟见肘", "明辨是非",
...
class DailyIdiomPlugin(Star):
    ...
    async def send_daily_idiom(self):
        ...
        try:
            ...
            logger.info(f"[每日成语] 随机选择的成语: {selected_idiom}")
    ...
    async def daily_task(self):
        ...
        while True:
            try:
                ...
                # 重新计算下一次的等待时间
                sleep_time = self.calculate_sleep_time()
                logger.info(f"[每日成语] 下一次推送将在 {sleep_time/3600:.2f} 小时后")
